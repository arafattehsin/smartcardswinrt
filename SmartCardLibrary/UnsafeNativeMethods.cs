...
namespace SmartCardLibrary
{
    ...
    public partial class UnsafeNativeMethods
    {
        ...
        [DllImport("winscard.dll")]
        public static extern int SCardConnect(SmartcardContextSafeHandle hContext, string szReaderName, int dwShareMode, int dwPrefProtocol, ref int phCard, ref int ActiveProtocol);

         [DllImport("winscard.dll", EntryPoint = "SCardEstablishContext", CharSet = CharSet.Unicode, 
             SetLastError = true)]
         static public extern uint EstablishContext(ScopeOption scope, IntPtr reserved1, 
         ...
         [DllImport("winscard.dll", EntryPoint = "SCardReleaseContext", CharSet = CharSet.Unicode, 
             SetLastError = true)]
         static public extern uint ReleaseContext(IntPtr context);

         [DllImport("winscard.dll", EntryPoint = "SCardListReaders", CharSet = CharSet.Unicode, 
             SetLastError = true)]
         static public extern uint ListReaders(SmartcardContextSafeHandle context, string groups, 
